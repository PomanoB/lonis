...
CREATE TABLE IF NOT EXISTS `news` (
  ...
  `caption` varchar(64) NOT NULL DEFAULT 'News #',
  `short` text NOT NULL,
  `text` text NOT NULL,
...
DROP TABLE IF EXISTS `styles_lang`;
CREATE TABLE IF NOT EXISTS `styles_lang` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `stylesid` int(10) NOT NULL,
  `lang` varchar(2) NOT NULL,
  `name` varchar(16) NOT NULL,
  PRIMARY KEY (`id`)
...
DROP TABLE IF EXISTS `achiev_lang`;
CREATE TABLE IF NOT EXISTS `achiev_lang` (
...
DROP TABLE IF EXISTS `players_achiev`;
...
DROP TABLE IF EXISTS `players_var`;
CREATE TABLE IF NOT EXISTS `players_var` (
  `playerId` int(11) NOT NULL,
  `key` varchar(50) NOT NULL,
...
DROP TRIGGER IF EXISTS `players_before_insert`;
...
DROP TRIGGER IF EXISTS `players_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `players_before_update` BEFORE UPDATE ON `players` FOR EACH ROW BEGIN 
SET NEW.`country` = (
	SELECT `code` FROM `geoip_whois`
	WHERE `ip_from` <= INET_ATON(NEW.`lastIp`) AND NEW.`lastIp` <> ''
	ORDER BY `ip_from` DESC LIMIT 1
...
DROP VIEW IF EXISTS `achievz`;
DROP TABLE IF EXISTS `achievz`;
CREATE VIEW `achievz` AS select `a`.`id` AS `id`,`a`.`type` AS `type`,`a`.`count` AS `count`,`a`.`icon` AS `icon`,`al`.`lid` AS `lid`,`al`.`achievid` AS `achievid`,`al`.`lang` AS `lang`,`al`.`name` AS `name`,`al`.`desc` AS `desc` from (`achiev` `a` join `achiev_lang` `al`) where (`a`.`id` = `al`.`achievid`);
...
DROP VIEW IF EXISTS `playerz`;
DROP TABLE IF EXISTS `playerz`;
CREATE VIEW `playerz` AS select `p`.`id` AS `id`,`p`.`name` AS `name`,`p`.`lastIp` AS `lastIp`,`p`.`email` AS `email`,`p`.`steam_id_64` AS `steam_id_64`,`p`.`country` AS `country`,`l`.`country_name` AS `countryName`,`l`.`locale_code` AS `lang`,(select count(0) from (`players_achiev` `pa` join `achiev` `a`) where ((`pa`.`achievId` = `a`.`id`) and (`a`.`count` = `pa`.`progress`) and (`pa`.`playerId` = `p`.`id`))) AS `achiev` from (`players` `p` left join `geoip_locations` `l` on((`p`.`country` = `l`.`country_iso_code`)));
